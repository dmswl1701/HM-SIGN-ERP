...
# 2026-08-11 추가 인계 (3) — 현장사진 올리기 (휴대폰 전용 화면)

`APP_BUILD` : `V45.0811.COPY53` → **`V46.0811.PHOTOUP`**
변경 파일: `HM-SIGN-ERP/public/index.html` 한 개.

## 왜 만들었나
대표 요청: *"핸드폰으로 하나씩 하려면 번거롭다."*
기능은 이미 있었지만(자동 현장 분리) 화면이 PC 기준이라 폰에서 쓰기 불편했다.

## 무엇인가
왼쪽 메뉴에 **`📸 현장사진 올리기`** 추가 (`state.view='photoup'`).
폰 세로화면 전용으로 크게 다시 그린 화면이고, **기능은 기존 것을 그대로 쓴다** —
`cmsAutoScanFiles` / `cmsAutoCluster` / `cmsAutoCreateDrafts` / `cmsStartUploadQueue`.

흐름: `사진 고르기(앨범에서 여러 장)` → `나눈 결과 확인(제목만)` → `올리기` → `진행률` → `공개는 따로`

- id·class 를 시공사례 탭과 **똑같이** 써서 `bindWebsiteAdmin()` 의 기존 핸들러가 그대로 붙는다.
  (`bindWebsiteAdmin` 은 화면과 무관하게 항상 호출되므로 새 바인딩 코드가 거의 필요 없었다)
- 업로드 진행률도 `cmsQBar_<key>` / `cmsQCount_<key>` id 를 재사용해
  `cmsQueueUpdateDom` 이 손대지 않고도 실시간으로 채워 준다.
- 폰 배려: 입력칸 **16px**(iOS 자동확대 방지), 버튼 최소 **56px**, 하단 `올리기` 버튼 **sticky**,
  썸네일 가로 스크롤, 가로 스크롤 0.
- 상태별 화면: 운영주소 아님 / 로그인 전(`로그인하러 가기`) / 고르기 전 / 읽는 중 / 확인 / 올리는 중 / 완료
- 완료 화면에 **"아직 홈페이지에는 안 보입니다"** 를 명시. 공개는 여전히 사람이 누른다.
- `ROLE_MENUS` 의 경영지원·디자이너·생산·설치에 `photoup` 추가
  (현장에서 사진을 찍는 건 생산·설치라서)

## 이번에 잡은 버그
- `bindWebsiteAdmin()` 안에 이미 `const pa`(cmsPrcAdd)가 있는데 새 코드가 `const pa` 를 또
  선언해 **스크립트 전체가 SyntaxError 로 죽었다.** 화면이 통째로 안 뜬다.
  → `pupLoginBtn` / `pupAdminBtn` / `pupMoreBtn` 로 이름 분리.
  **교훈: 이 파일은 함수 하나가 아주 길다. 짧은 변수명(`pa`,`pm`,`pl`)을 새로 쓰지 말 것.**
- 미리보기 blob 주소를 화면에서 지우기 **전에** 반납해 콘솔에 `ERR_FILE_NOT_FOUND` 가
  잔뜩 찍혔다 → `render()` 뒤에 반납하도록 순서 변경. (업로드는 대기열이 만든 별도 주소를 쓰므로 무관)

## 검증한 것 [사실]
375×812(모바일) + 데스크톱 양쪽에서 실제 브라우저 실행:
- 상태 7가지 모두 정상 렌더. 가로 스크롤 0. 제목칸 16px, 올리기 버튼 56px, 하단바 sticky 확인
- 사진 5~10장 투입 → 현장 카드 생성 → `현장 N개 올리기` → 진행줄·진행바 표시 →
  완료 시 `✅ 다 올렸어요` + 장수 정확, **공개된 것 0건**
- `사진 더 올리기` / `확인하고 홈페이지에 공개하기` 버튼 동작 확인
- **회귀:** 시공사례 탭(패널 2·자동분리존·역할버튼 8), 문구 편집 탭(묶음 10·편집칸 53),
  배너 탭, 그 외 6개 화면 렌더 정상. 콘솔 JS 에러 0.

## 검증 못 한 것 [사실]
- **실제 폰에서 열어보지 못했다.** 데스크톱 브라우저의 모바일 뷰포트로만 확인했다.
  iOS Safari 의 파일 선택 동작(앨범 다중선택)은 기기에서 확인이 필요하다.
- 실제 업로드(CMS Worker 통신)는 로그인·운영 origin 이 필요해 여전히 미확인.
- 스크린샷은 브라우저가 프레임을 그리지 않아 못 찍었다.

## 다음 (대표와 합의됨 2026-08-11)
**구글 드라이브에 폴더 하나를 만들고 그 폴더만 읽어 사진을 가져오는 방식**으로 확장.
"일정 폴더에서만 추출" 이 대표 요구사항이며, 구글 OAuth 도 사용자가 고른 것만
접근하는 방식(`drive.file`)이라 잘 맞는다. ⚠ 구글 **포토**는 2025년 API 제한으로 불가,
반드시 **드라이브**여야 한다.
...
# 2026-08-11 추가 인계 (4) — 화면 구성 · 구글 드라이브(베타)

`APP_BUILD` : `V46.0811.PHOTOUP` → **`V47.0811.LAYOUT`**
변경 저장소 **2개**: `HM-SIGN-ERP` + `hminterior-official`

## 1) 화면 구성 — 섹션 켜기/끄기 + 순서 바꾸기 [검증 완료]
홈페이지 관리에 **`화면 구성`** 탭 추가. **Supabase 스키마 변경 없음** —
`pages.layout.<페이지>` 배열 하나로만 처리한다.

| 페이지 | 다룰 수 있는 섹션 |
|---|---|
| 홈 | 약속 · 시공사례 · 강점 3가지 · 신뢰 |
| 회사소개 | 믿는 것 · 기준 4가지 · 맨 아래 상담 안내 |
| 프로세스 | 도입 문구 · 진행 4단계 · 약속 3가지 · 맨 아래 상담 안내 |

첫 화면(hero)·머리말·메뉴는 `data-section` 이 없어 손대지 않는다.

**홈페이지 쪽 동작** (`site.js` 의 `화면 구성` 블록):
- `pages.layout.<page>` 가 배열이면 그 순서대로 다시 배치하고, 목록에 없는 섹션은 `hidden`
- **값이 없으면 아무것도 하지 않는다** — HTML 에 적힌 순서 그대로 (기존 동작 보존)
- 모르는 key 가 섞여 있어도 무시하고 넘어간다
- `site.css` 에 `[data-section][hidden]{display:none!important}` 한 줄 추가.
  섹션에 `display` 를 주는 규칙들이 있어 `!important` 가 필요하다.

**ERP 쪽:** ▲▼ 로 순서, `끄기/켜기` 로 표시 여부. 껐다 켜면 **원래 기본 자리로** 돌아간다.
`이 페이지 원래대로` 는 값을 비워 기본 순서로 되돌린다.

## 2) 시공사례 예시 목록도 문구 편집 [검증 완료]
공개된 시공사례가 0건일 때 대신 나오는 예시 2개(`워시팡팡`/`올인 베이커리`)에
태그를 달았다. 편집 가능한 문구 **53 → 57곳**.

## 3) 구글 드라이브에서 사진 가져오기 [★ 부분 검증]
대표 요청: *"구글 드라이브 하나 만들어서 **일정 폴더에서만** 사진 추출."*

- **설정 → 구글 드라이브** 에서 `클라이언트 ID` + `API 키` 를 넣으면
  `현장사진 올리기` 화면에 버튼이 생긴다. **값이 없으면 버튼 자체가 없다.**
- 권한은 **`drive.file`** — 드라이브 전체가 아니라 **선택창에서 고른 사진만** 접근한다.
- 원본을 그대로 받으므로 **EXIF(촬영시각·GPS)가 살아 있다.**
  ⚠ 구글 **포토**는 2025년 API 제한으로 불가. **드라이브**여야 한다.
- 안전장치: 구글 스크립트는 **버튼을 누른 순간에만** 불러온다. 전부 try/catch.
  실패해도 기존 사진 올리기에는 아무 영향이 없다.

**검증한 것 [사실]:**
- 구글 스크립트 2개(`gsi/client`, `api.js`) + `picker` 모듈 **실제 로드 성공(885ms)**
- 코드가 쓰는 API 심볼이 전부 실재함을 확인:
  `google.accounts.oauth2.initTokenClient` / `google.picker.PickerBuilder` /
  `DocsView` / `ViewId.DOCS_IMAGES`(=`docs-images`) / `Feature.MULTISELECT_ENABLED`
- `PickerBuilder` 호출 사슬 전체가 **실제 API 상대로 예외 없이 build() 됨**
- 연결값 없음 → 버튼 없음 / 값 있음 → 버튼 생김 / 해제 동작 확인

**검증 못 한 것 [사실 — 구글 계정이 있어야 가능]:**
- OAuth 동의 화면과 실제 토큰 발급
- 드라이브에서 실제 파일 내려받기
- ERP 주소가 `승인된 자바스크립트 원본` 에 제대로 등록됐는지
→ **대표가 값을 넣고 처음 누를 때 오류가 날 수 있다. 오류 문구를 받아 고칠 것.**

## 검증한 것 (1·2번) [사실]
- 키 대조: 홈페이지 태그 **57** ↔ ERP 편집칸 **57**, 어긋난 키 0, 중복 0
- 홈·회사소개·프로세스에서 순서 뒤집기·끄기·복원·모르는key무시·layout없음 **5가지 모두 통과**
- 끈 섹션은 `display:none` 실측 확인. hero·머리말은 그대로 유지됨
- ERP: ▲▼ 이동, 끄기/켜기(원래 자리 복귀), 맨위/맨아래 버튼 비활성, `원래대로` 동작
- 회귀: CMS 탭 7개 + 전체 화면 16개 렌더 정상, 콘솔 JS 에러 0

---

---

# 2026-08-11 추가 인계 (5) — 인트로 v3 · 업로드 체감 개선

`APP_BUILD` : `V47.0811.LAYOUT` → **`V48.0811.WAKE`**
변경 저장소 **2개**: `HM-SIGN-ERP` + `hminterior-official`

## 구글 드라이브 — 대표가 연결 성공 [사실]
2026-08-11 대표 확인: **"잘 연결했고 잘 돼."**
(4)번 인계에서 "미검증" 이라 적었던 부분은 이제 **실제로 동작 확인됨.**

## 1) 브랜드 인트로 v3 — "좀 더 화려하게" [검증 완료]
대표 요청. **총 길이는 그대로** 두고(4.15s / 모바일 2.99s) 장면만 풍성하게 했다.

추가된 것:
- **점등 깜빡임** — `hm-lit` / `hm-lit-amber` 를 8단계로 바꿔, 네온·형광등이 켜질 때처럼
  **지직거리다 안정**된다. 점등 시간 .55s → .68s
- **점등 플래시** (`.hm-intro-flash`) — 1.02s 에 화면 전체가 확 밝아졌다 두 번 부풀며 잦아든다
- **빛 스윕** (`.hm-intro-logo::after`) — 2.02s 에 글자 위로 빛이 지나간다 (금속 간판 반사)
- **로고 반사광** (`.hm-intro-logo::before`) — 1.72s 부터 글자 아래가 은은하게 물든다
- **도면 꺾쇠 4개** (0.08s) · **간판 폭 치수선** (0.62s) — 설계도 정체성 강화.
  둘 다 `.hm-intro-lines` 안에 있어 2.40s 에 부모와 함께 사라진다

**검증 [사실]:** 실제 브라우저에서 computed style 로 타임라인 전수 확인 —
데스크톱 4.15s / 모바일 2.988s **총 길이 변화 없음**, 각 단계 delay 가 설계값과 정확히 일치.
`hm-lit`·`hm-lit-amber` 8단계, `hm-flash` 6단계 키프레임이 제대로 파싱됨을 CSSOM 으로 확인.
`prefers-reduced-motion` 규칙 3개 그대로 살아 있음. 가로 스크롤 0.
⚠ **브라우저가 프레임을 안 그려 실제 움직임은 여전히 못 봤다.** 타이밍·연결만 검증했다.

## 2) 사진 업로드 체감 개선 [검증 완료]
대표: *"뭔가 사진 올리기 불편한데 어쩔 수 없는 부분 같기도."*
→ 짐작이 아니라 실제로 고칠 수 있는 원인이 있었다.

- **화면 꺼짐 방지 (Screen Wake Lock)** — 폰에서 사진을 많이 올릴 때 화면이 저절로 꺼지면
  브라우저가 멈춰 **업로드가 끊긴다.** 업로드가 도는 동안만 화면을 깨워 둔다.
  화면을 껐다 켜면 lock 이 풀리므로 `visibilitychange` 에서 다시 잡는다.
  **지원하지 않는 기기에서는 조용히 넘어간다** (구형 iOS 등 — 미지원 경로도 검증했다).
  ⚠ 사용자가 **직접** 화면을 끄거나 다른 앱으로 넘어가는 것은 막지 못한다. 그건 브라우저 권한 밖이다.
- **동시 업로드 2 → 3줄.** 더 늘리면 폰에서 canvas 이미지 변환이 몰려 오히려 느려진다.
- **남은 시간 표시** (`cmsQueueEta`) — 실제로 올린 속도로 어림한다.
  2장 미만이면 표시하지 않는다(엉뚱한 값이 나온다). 45초 미만은 "곧 끝나요".

**검증 [사실]:** wake lock 요청·release 리스너 등록·종료 시 1회 해제 확인.
동시 처리 최대 **3개** 실측. 진행률 문구에 남은 시간이 붙는 것 확인
(`전체 6 · 완료 2 · 실패 0 · 업로드 중 · 곧 끝나요`).
wakeLock 미지원 기기에서도 업로드가 정상 완료되는 것 확인.
회귀: 전체 화면 16개 + CMS 탭 7개 렌더 정상.

**아직 남은 불편 (정직하게):** 사진 1장마다 원본 + 2000px + 640px **3개 파일**을 올린다.
이건 CMS Worker 의 계약이라 ERP 만으로는 못 줄인다. 100장이면 300개 요청이다.
더 빠르게 하려면 Worker 쪽에서 파생 이미지를 만들게 바꿔야 하는데,
Worker 에 canvas 가 없어 **유료 기능(Cloudflare Images)** 이 필요하다.
