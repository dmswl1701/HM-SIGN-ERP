...
# 2026-08-11 추가 인계 (7) — ★ 문구 저장이 실제로 깨져 있었다 · 편집 편의 개선

`APP_BUILD` : `V49.0811.NODRIVE` → **`V50.0811.EASYEDIT`**

## ★★★ 최종 결론 (2026-08-11, 저장 시험으로 확인) — 저장은 되고 있었다
대표가 [문구 저장 시험하기] 를 눌러 얻은 실제 결과:
```
{payload,status} : 통신오류(응답을 받지 못함) · Failed to fetch  → 서버에는 들어감
```
**PUT 은 서버에서 정상 처리되어 값이 저장되는데, 그 응답만 브라우저에 닿지 못한다.**
ERP 는 `fetch` 가 던진 것을 보고 "실패" 로 처리했을 뿐이다.
(왜 `pages` 응답만 유실되는지는 Worker 쪽 문제다. ERP 로는 원인을 더 좁힐 수 없다.)

**고친 방법:** PUT 이 던지면 **다시 읽어서 값이 실제로 들어갔는지 확인**하고,
들어갔으면 성공으로 처리한다(`cmsVerifySaved`). 안 들어갔을 때만 실패로 남긴다.
→ Worker 를 못 고쳐도 문구 편집·화면 구성이 정상 동작한다.

⚠ 이전 시험 도구에 버그가 있었다: "행이 있는지(200)" 만 봐서, 원래 있던 행 때문에
항상 성공으로 보였다. 지금은 **보낸 값과 읽은 값을 비교**한다.

**틀린 진단을 두 번 했다 (같은 길로 새지 말 것):**
1. "Worker `contentKeys` 에 `pages` 누락" → 틀림. 처음부터 들어 있었다.
2. "요청 모양이 `{payload:...}` 가 아니어서" → 모양은 맞았고 그래도 실패했다.
진짜 원인은 **응답 유실**이었다.

---

## (앞선 조사 기록) `pages` 키 저장 실패 — 요청 모양 가설 [결과적으로 원인 아님]
세션 내내 "미확인" 이던 항목이다. 대표가 저장을 눌렀을 때:
```
6개는 저장됐고, 1개는 실패했습니다.
    pages → 통신오류
```

**틀린 추측을 먼저 적어 둔다 (다음 사람이 같은 길로 새지 않도록):**
처음에는 "Worker 의 `contentKeys` 에 `pages` 가 없어서" 라고 판단했다. **틀렸다.**
대표가 대시보드를 열어 확인해 준 실제 코드:
```js
const contentKeys = new Set(['company','banner','services','process','contact','seo','pages']);
```
`pages` 는 처음부터 허용되어 있었다. **Worker 는 손댈 필요가 없다.**

**진짜 원인 [근거 있음]:**
- 공개 API(`/api/v1/public/site`) 응답에 `pages` 키가 아예 없다 = **한 번도 저장에 성공한 적이 없다**
- 성공하는 6개 키는 서버에서 행(row)을 받아 온 적이 있어 `cmsRows[key]` 가 채워져 있다
- `cmsWrapValue(row, value)` 는 행이 있으면 `{payload: 값, status:'published'}` 로 감싸지만,
  **행이 없으면** `Object.assign({}, value, {status})` 로 **값을 통째로 펼쳐서** 보냈다
  → `{home:{...}, about:{...}, status:'published'}`
- Worker 가 기대하는 모양이 아니라 처리 중 오류가 났고, 그 오류 응답에는 CORS 헤더가
  붙지 않아 브라우저가 차단했다 → HTTP 400 이 아니라 **fetch 자체가 throw** → "통신오류"

**고친 것 (ERP 코드):** `cmsRowField()` 를 추가해, 이미 받아 본 다른 행에서
값이 담기는 칸 이름(`payload`)을 알아내고 **새 키도 같은 모양으로** 보낸다.
행이 하나도 없을 때는 `payload` 로 기본 처리한다.

⚠ **아직 대표가 실제로 저장을 눌러 확인하지는 못했다.** 모양이 맞는 것까지만 검증했다.
다음 세션에서 제일 먼저 확인할 것.

## 1) 저장 실패 안내 [검증 완료]
`pages → 통신오류` 한 줄로는 무엇을 해야 할지 알 수 없다.
`cmsSaveHint(key)` 로 키 이름을 사람 말로 바꾸고(`pages` → `문구 편집 · 화면 구성`),
그 항목이 **처음 저장하는 것인지**에 따라 다른 안내를 보여준다.
⚠ 원인을 단정하는 문구는 넣지 않았다 — 처음에 단정했다가 틀렸기 때문이다.

## 2) 사인 종류 · 진행 절차 편집 불편 해소 [검증 완료]
대표: *"사인종류나 절차는 추가만 가능하고 삭제 변경 안 되는 거 불편해."*

실제 원인: 목록이 **0개**라 홈페이지는 기본값을 보여주는데, 그걸 고치려면
**다섯 개를 처음부터 타이핑**해야 했다. (항목을 넣은 뒤에는 수정·삭제가 원래 됐다.)

- **`기본 5가지 불러오기` / `기본 4단계 불러오기`** — 홈페이지에 지금 나오는 내용을
  그대로 목록에 넣어 준다. 바로 고치거나 지우면 된다. (`CMS_DEFAULT_SERVICES` / `CMS_DEFAULT_PROCESS`)
- **`전부 지우고 기본으로`** — 목록을 비워 기본값으로 되돌린다
- **▲▼ 순서 바꾸기** 추가 (맨 위/맨 아래는 비활성)
- **진행 절차에 설명칸 추가** — 이게 조용한 버그였다. 예전 구조는 절차를 **문자열**로만
  저장해서, ERP 에서 단계를 넣는 순간 **홈페이지의 단계 설명이 통째로 사라졌다.**
  이제 `{name,text}` 로 저장한다. `cmsStepNormalize()` 가 옛 문자열 값도 그대로 읽어 준다.
  ⚠ 절차를 문자열로 되돌리지 말 것.

**검증 [사실]:** 기본값 불러오기(5개/4단계, 설명 포함) → ▲▼ 이동 → 이름·설명 수정 →
삭제 → 저장될 JSON 모양까지 실제로 눌러 확인. 옛 문자열 절차도 정상 변환 확인.
회귀: CMS 탭 7개 + 전체 화면 16개 렌더 정상, 콘솔 JS 에러 0.
...
# 2026-09-19 추가 인계 — 홈페이지 파일 대량 삭제 복구 · 미리보기 사진 화질

집 PC에서 작업. 대표 증상: *"특정 버튼(회사소개) 누르면 디자인 이상해짐 · 사진 화질 깨짐"*

## 1) 홈페이지 — 9/2 커밋이 파일 42개를 지웠다 [사실]
`hminterior-official` `2ed6b52` (9/2, "관리자 화면을 홈페이지 구성 그대로 다시 짰다")는
admin·index·home-v18.js 만 고치는 커밋인데 **파일 42개를 함께 삭제**했다. 설명에 언급이 없어 실수로 본다.
운영 사이트에서 실측한 결과:

| 증상 | 원인 파일 |
|---|---|
| 회사소개가 글자만 뜸 (디자인 전무) | `about-v18.css/js` 404 |
| 홈 첫 슬라이드 회색 | `home-hero.webp` 404 |
| **시공사례 상세 `/works/{slug}` 전부 404** | `_redirects`·`project.js` 삭제 |
| 구글 서치콘솔 소유확인 308 | `_redirects` 200 규칙 삭제 |
| 파비콘·움직임·예전 5페이지 로고·카톡 미리보기 이미지 | `favicon.svg` `motion.*` `brand-lockup-v1.css` `hero.jpg` |

**복구:** 브랜치 `fix/restore-deleted-assets-20260919` — 지금 코드가 부르는 파일 + Pages 설정 15개만
`2ed6b52^` 에서 바이트 그대로 되살림. 아무도 안 부르는 옛 버전(home-v3~v16 등)은 되살리지 않음.
로컬 정적 서버에서 10개 경로의 참조 파일 전부 200, 스크립트 오류 0 확인.

⚠ **교훈:** 커밋 전에 `git status` 로 의도하지 않은 삭제(D)가 없는지 볼 것.

## 2) ERP — 사진 축소 화질 [사실]
`cmsImageToWebp` 가 한 번의 `drawImage` 로 줄였다(기본 보간). 폰 원본 4032px → 640px(6.3배)에서
간판 글씨·LED 가 계단처럼 깨진다. `cmsDrawDownscaled()` 추가 — 절반씩 나눠 줄이고 매 단계 `imageSmoothingQuality='high'`.

실제 업로드 조건(JPEG → WebP) 측정, 면적 평균 대비 오차:
- 미리보기 640px: 7.88 → **5.09 (35% 감소)**, 용량 26KB → 20KB
- 대표 2000px: 3.01 → 3.02 (**변화 없음** — 2배 축소라 원래 문제 없었다)

빌드 패치(`scripts/*.mjs`)가 찾는 문자열 11개의 등장 횟수 변화 0 확인. 패치의 모든 업로드 경로가
`cmsUploadAsset` → `cmsImageToWebp` 를 거치므로 전부 적용된다. **새로 올리는 사진부터** 적용된다.

## 3) 남은 화질 문제 (홈페이지 표시 쪽) [사실 — 실측, 미수정]
- `home-v18.js` 사인 종류 카드(`.kind`)가 `thumbnail_url`(640px)을 **먼저** 쓴다.
  데스크톱 208x560 세로 칸 → 레티나 1.75배, 폰 약 2.6배 확대되어 흐리다. `cover_url` 우선 또는 크기 기반 선택 필요.
- 히어로는 2000px 대표사진을 전체 화면에 cover 로 깐다. 세로 사진은 높이 약 60% 가 잘리고 레티나에서 1.9배 확대.
- 둘 다 CMS 가 운영 origin 만 허용해 **로컬·프리뷰에서는 실사진 검증이 안 된다.** 운영에서 확인하며 고칠 것.
